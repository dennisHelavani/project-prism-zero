...
/**
 * Trigger document generation via internal API route (subprocess).
 * This is fire-and-forget - the Python process updates Supabase when done.
 */
async function triggerDocGeneration(submissionId: string): Promise<void> {
  ...
  try {
    // Call our own internal API route which spawns the subprocess
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:9002';
    const res = await fetch(`${siteUrl}/api/generate-document`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ submission_id: submissionId }),
      // Important: don't wait indefinitely, but we do want to ensure the request is sent
      signal: AbortSignal.timeout(5000)
    });

    if (!res.ok) {
      const errorText = await res.text();
      console.error(`Doc generation trigger failed: ${res.status} - ${errorText}`);
      // Note: We don't update Supabase here because we might have timed out waiting 
      // for the trigger, but the process might still be running.
    } else {
      console.log('Doc generation process verified started.');
    }
  } catch (err) {
    console.error('Failed to trigger doc generator:', err);
    // Don't error out the user response just because background trigger failed
  }
}
